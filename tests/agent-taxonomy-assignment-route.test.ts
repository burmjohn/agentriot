...
const assignAgentTaxonomy = vi.fn();

vi.mock("@/lib/ingestion/auth", () => ({
  authenticateIngestionRequest,
}));

vi.mock("@/lib/ingestion/agent-taxonomy-assignment", () => ({
  assignAgentTaxonomy,
}));

describe("agent taxonomy assignment ingestion route", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns a stable unauthorized error when the request has no valid API key", async () => {
    ...
    const { POST } = await import("@/app/api/v1/ingest/agent-taxonomy/route");
    const response = await POST(
      new Request("http://localhost:3011/api/v1/ingest/agent-taxonomy", {
        method: "POST",
        body: JSON.stringify({ agentId: "agent-1", taxonomyTermIds: ["term-a"] }),
    ...
    expect(assignAgentTaxonomy).not.toHaveBeenCalled();
  });

  it("replaces agent taxonomy assignments when auth and payload are valid", async () => {
    authenticateIngestionRequest.mockResolvedValue({
      ok: true,
      key: { id: "key-1" },
    });
    assignAgentTaxonomy.mockResolvedValue({
      kind: "agent-taxonomy",
      agentId: "agent-1",
    ...
    expect(assignAgentTaxonomy).toHaveBeenCalledWith({
      apiKeyId: "key-1",
  ...
  it("returns a stable conflict envelope for mismatched idempotent payloads", async () => {
    ...
    assignAgentTaxonomy.mockRejectedValue(
