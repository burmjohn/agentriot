...
const assignPromptTaxonomy = vi.fn();

vi.mock("@/lib/ingestion/auth", () => ({
  authenticateIngestionRequest,
}));

vi.mock("@/lib/ingestion/prompt-taxonomy-assignment", () => ({
  assignPromptTaxonomy,
}));

describe("prompt taxonomy assignment ingestion route", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns a stable unauthorized error when the request has no valid API key", async () => {
    ...
    const { POST } = await import("@/app/api/v1/ingest/prompt-taxonomy/route");
    const response = await POST(
      new Request("http://localhost:3011/api/v1/ingest/prompt-taxonomy", {
        method: "POST",
        body: JSON.stringify({ promptId: "prompt-1", taxonomyTermIds: ["term-a"] }),
    ...
    expect(assignPromptTaxonomy).not.toHaveBeenCalled();
  });

  it("replaces prompt taxonomy assignments when auth and payload are valid", async () => {
    authenticateIngestionRequest.mockResolvedValue({
      ok: true,
      key: { id: "key-1" },
    });
    assignPromptTaxonomy.mockResolvedValue({
    ...
    expect(assignPromptTaxonomy).toHaveBeenCalledWith({
      apiKeyId: "key-1",
  ...
  it("returns a stable conflict envelope for mismatched idempotent payloads", async () => {
    ...
    assignPromptTaxonomy.mockRejectedValue(
