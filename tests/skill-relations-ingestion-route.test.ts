...
const assignSkillRelations = vi.fn();

vi.mock("@/lib/ingestion/auth", () => ({
  authenticateIngestionRequest,
}));

vi.mock("@/lib/ingestion/skill-relations-ingestion", () => ({
  assignSkillRelations,
}));

describe("skill relations ingestion route", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns a stable unauthorized error when the request has no valid API key", async () => {
    ...
    const { POST } = await import("@/app/api/v1/ingest/skill-relations/route");
    const response = await POST(
      new Request("http://localhost:3011/api/v1/ingest/skill-relations", {
        method: "POST",
        body: JSON.stringify({
          skillId: "skill-1",
          agentIds: [],
    ...
    expect(assignSkillRelations).not.toHaveBeenCalled();
  });

  it("replaces skill relations when auth and payload are valid", async () => {
    authenticateIngestionRequest.mockResolvedValue({
      ok: true,
      key: { id: "key-1" },
    });
    assignSkillRelations.mockResolvedValue({
    ...
    expect(assignSkillRelations).toHaveBeenCalledWith({
      apiKeyId: "key-1",
  ...
  it("returns a stable conflict envelope for mismatched idempotent payloads", async () => {
    ...
    assignSkillRelations.mockRejectedValue(
