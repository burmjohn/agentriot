...
const assignContentRelations = vi.fn();

vi.mock("@/lib/ingestion/auth", () => ({
  authenticateIngestionRequest,
}));

vi.mock("@/lib/ingestion/content-relations-ingestion", () => ({
  assignContentRelations,
}));

describe("content relations ingestion route", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns a stable unauthorized error when the request has no valid API key", async () => {
    ...
    const { POST } = await import("@/app/api/v1/ingest/content-relations/route");
    const response = await POST(
      new Request("http://localhost:3011/api/v1/ingest/content-relations", {
        method: "POST",
        body: JSON.stringify({
          contentId: "content-1",
          agentIds: [],
    ...
    expect(assignContentRelations).not.toHaveBeenCalled();
  });

  it("replaces content relations when auth and payload are valid", async () => {
    authenticateIngestionRequest.mockResolvedValue({
      ok: true,
      key: { id: "key-1" },
    });
    assignContentRelations.mockResolvedValue({
    ...
    expect(assignContentRelations).toHaveBeenCalledWith({
      apiKeyId: "key-1",
  ...
  it("returns a stable conflict envelope for mismatched idempotent payloads", async () => {
    ...
    assignContentRelations.mockRejectedValue(
