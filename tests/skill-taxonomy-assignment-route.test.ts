...
const assignSkillTaxonomy = vi.fn();

vi.mock("@/lib/ingestion/auth", () => ({
  authenticateIngestionRequest,
}));

vi.mock("@/lib/ingestion/skill-taxonomy-assignment", () => ({
  assignSkillTaxonomy,
}));

describe("skill taxonomy assignment ingestion route", () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
  ...
  it("returns a stable unauthorized error when the request has no valid API key", async () => {
    ...
    const { POST } = await import("@/app/api/v1/ingest/skill-taxonomy/route");
    const response = await POST(
      new Request("http://localhost:3011/api/v1/ingest/skill-taxonomy", {
        method: "POST",
        body: JSON.stringify({ skillId: "skill-1", taxonomyTermIds: ["term-a"] }),
    ...
    expect(assignSkillTaxonomy).not.toHaveBeenCalled();
  });

  it("replaces skill taxonomy assignments when auth and payload are valid", async () => {
    authenticateIngestionRequest.mockResolvedValue({
      ok: true,
      key: { id: "key-1" },
    });
    assignSkillTaxonomy.mockResolvedValue({
      kind: "skill-taxonomy",
      skillId: "skill-1",
    ...
    expect(assignSkillTaxonomy).toHaveBeenCalledWith({
      apiKeyId: "key-1",
  ...
  it("returns a stable conflict envelope for mismatched idempotent payloads", async () => {
    ...
    assignSkillTaxonomy.mockRejectedValue(
