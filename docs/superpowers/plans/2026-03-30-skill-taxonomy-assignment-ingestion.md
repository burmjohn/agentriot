# Skill taxonomy assignment ingestion implementation plan
...
**Goal:** Add authenticated replace-all taxonomy assignment for skill records
at `POST /api/v1/ingest/skill-taxonomy`.

**Architecture:** Mirror the prompt taxonomy assignment ingestion route, but
target the skill taxonomy join model and the `skills:write` scope. Add a
dedicated `skill-taxonomy-assignment` service, a single route that reuses
...
- Create: `lib/ingestion/skill-taxonomy-assignment.ts`
- Create: `app/api/v1/ingest/skill-taxonomy/route.ts`
- Create: `tests/skill-taxonomy-assignment-service.test.ts`
- Create: `tests/skill-taxonomy-assignment-route.test.ts`
...
- [ ] Write the failing service test for happy path, clear-all, replay,
  conflict, wrong scope, missing skill, invalid payload, and non-object JSON.
- [ ] Run `pnpm test -- tests/skill-taxonomy-assignment-service.test.ts` and
  verify it fails because the service does not exist yet.
- [ ] Implement the minimal service using `skillTaxonomyTerms`,
...
- [ ] Run `pnpm test -- tests/skill-taxonomy-assignment-route.test.ts` and
...
- [ ] Add the new POST-only route card for `/api/v1/ingest/skill-taxonomy`.
- [ ] Update the “Planned next” copy so it now points at broader relation
  mutation rather than more taxonomy symmetry.
...
- [ ] Open a PR against `main` titled `Add skill taxonomy assignment ingestion`.
