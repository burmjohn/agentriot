# Agent relations ingestion implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an idempotent `POST /api/v1/ingest/agent-relations` endpoint that replaces agent-to-prompt and agent-to-skill joins in one transaction.
...
- Create: `lib/ingestion/agent-relations-ingestion.ts`
- Create: `app/api/v1/ingest/agent-relations/route.ts`
- Create: `tests/agent-relations-ingestion-service.test.ts`
- Create: `tests/agent-relations-ingestion-route.test.ts`
...
**Files:**
- Create: `tests/agent-relations-ingestion-service.test.ts`
- Reference: `lib/ingestion/content-relations-ingestion.ts`
...
```bash
pnpm test -- tests/agent-relations-ingestion-service.test.ts
```

Expected:

- FAIL because `@/lib/ingestion/agent-relations-ingestion` does not exist yet
...
- replace `agent_prompts`
- replace `agent_skills`
...
- `assignAgentRelations({ apiKeyId, idempotencyKey, payload })`
...
```bash
pnpm test -- tests/agent-relations-ingestion-route.test.ts
```

Expected:

- FAIL because `app/api/v1/ingest/agent-relations/route.ts` does not exist yet

## Task 4: Implement the route

**Files:**
- Create: `app/api/v1/ingest/agent-relations/route.ts`
- Reference: `app/api/v1/ingest/content-relations/route.ts`
...
```bash
pnpm test -- tests/agent-relations-ingestion-service.test.ts tests/agent-relations-ingestion-route.test.ts
```
...
```bash
git add lib/ingestion/agent-relations-ingestion.ts \
  ...
  app/api/page.tsx
git commit -m "Add agent relations ingestion"
```
...
gh pr create --base main --head <feature-branch> --title "Add agent relations ingestion"
```
