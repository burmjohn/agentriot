# Content relations ingestion implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an idempotent `POST /api/v1/ingest/content-relations` endpoint that replaces content-to-agent, content-to-prompt, and content-to-skill joins in one transaction.

**Architecture:** Reuse the existing ingestion contract and relation-write helpers. Implement one service that validates and normalizes the full payload, applies all three join-table replacements atomically, and records the ingestion event for replay and conflict handling.
...
- Create: `lib/ingestion/content-relations-ingestion.ts`
- Create: `app/api/v1/ingest/content-relations/route.ts`
- Create: `tests/content-relations-ingestion-service.test.ts`
- Create: `tests/content-relations-ingestion-route.test.ts`
...
**Files:**
- Create: `tests/content-relations-ingestion-service.test.ts`
- Reference: `lib/ingestion/content-taxonomy-assignment.ts`

- [ ] **Step 1: Write the failing test file for the service**

Add tests for:

- success with all three relation families
...
```bash
pnpm test -- tests/content-relations-ingestion-service.test.ts
```

Expected:

- FAIL because `@/lib/ingestion/content-relations-ingestion` does not exist yet
...
- replace `content_agents`
- replace `content_prompts`
- replace `content_skills`
...
- `assignContentRelations({ apiKeyId, idempotencyKey, payload })`
...
```bash
pnpm test -- tests/content-relations-ingestion-route.test.ts
```

Expected:

- FAIL because `app/api/v1/ingest/content-relations/route.ts` does not exist yet

## Task 4: Implement the route

**Files:**
- Create: `app/api/v1/ingest/content-relations/route.ts`
- Reference: `app/api/v1/ingest/content-taxonomy/route.ts`
...
```bash
pnpm test -- tests/content-relations-ingestion-service.test.ts tests/content-relations-ingestion-route.test.ts
```
...
```bash
git add lib/ingestion/content-relations-ingestion.ts \
  ...
  app/api/page.tsx
git commit -m "Add content relations ingestion"
```
...
gh pr create --base main --head <feature-branch> --title "Add content relations ingestion"
```
