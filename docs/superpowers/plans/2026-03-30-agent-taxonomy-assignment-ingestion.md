# Agent taxonomy assignment ingestion implementation plan
...
**Goal:** Add authenticated replace-all taxonomy assignment for agent records at
`POST /api/v1/ingest/agent-taxonomy`.

**Architecture:** Mirror the content taxonomy assignment ingestion route, but
target the agent taxonomy join model and the `agents:write` scope. Add a
dedicated `agent-taxonomy-assignment` service, a single route that reuses
...
- Create: `lib/ingestion/agent-taxonomy-assignment.ts`
- Create: `app/api/v1/ingest/agent-taxonomy/route.ts`
- Create: `tests/agent-taxonomy-assignment-service.test.ts`
- Create: `tests/agent-taxonomy-assignment-route.test.ts`
...
- [ ] Write the failing service test for happy path, clear-all, replay,
  conflict, wrong scope, missing agent, invalid payload, and non-object JSON.
- [ ] Run `pnpm test -- tests/agent-taxonomy-assignment-service.test.ts` and
  verify it fails because the service does not exist yet.
- [ ] Implement the minimal service using `agentTaxonomyTerms`,
...
- [ ] Run `pnpm test -- tests/agent-taxonomy-assignment-route.test.ts` and
...
- [ ] Add the new POST-only route card for `/api/v1/ingest/agent-taxonomy`.
- [ ] Update the “Planned next” copy so it now points at prompt and skill
  taxonomy assignment plus broader relation mutation.
...
- [ ] Open a PR against `main` titled `Add agent taxonomy assignment ingestion`.
