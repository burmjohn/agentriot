# AgentRiot prompt taxonomy assignment ingestion design

Date: March 30, 2026
Project: AgentRiot
Scope: Authenticated replace-all taxonomy assignment for prompt records

## Goal

Add the next graph-mutation ingestion surface after agent taxonomy
assignment: `POST /api/v1/ingest/prompt-taxonomy`.

This slice lets trusted publishers replace the taxonomy term assignments for a
single prompt record through the existing API key flow without widening the
branch into skill mutation or cross-entity relation writes.

## Product decision

Prompt-only taxonomy assignment ships before broader multi-entity relation
mutation.

This is the right next move because:

- it extends the proven taxonomy-mutation pattern to the next self-contained
  graph entity
- prompts already have a stable admin taxonomy workflow to mirror
- it reuses the existing `prompt_taxonomy_terms` join model directly
- it keeps the branch narrow enough to review cleanly before expanding to
  skills
...
- an empty array is valid and clears the prompt record’s taxonomy joins
- all referenced taxonomy terms must exist
- all referenced taxonomy terms must have `scope = prompt`
...
- `entity: "ingest:prompt-taxonomy"`
- `data.kind: "prompt-taxonomy"`
- `data.promptId`
...
5. The service verifies the prompt record exists.
...
- `lib/ingestion/prompt-taxonomy-assignment.ts`
...
- verify taxonomy term existence and `scope = prompt`
...
- `prompt_taxonomy_terms` for join storage
...
- one request describes the full desired prompt taxonomy state
...
- trusted publishers can replace prompt taxonomy assignments with
  `POST /api/v1/ingest/prompt-taxonomy`
