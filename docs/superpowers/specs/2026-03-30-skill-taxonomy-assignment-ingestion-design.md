# AgentRiot skill taxonomy assignment ingestion design

Date: March 30, 2026
Project: AgentRiot
Scope: Authenticated replace-all taxonomy assignment for skill records

## Goal

Add the next graph-mutation ingestion surface after prompt taxonomy
assignment: `POST /api/v1/ingest/skill-taxonomy`.

This slice lets trusted publishers replace the taxonomy term assignments for a
single skill record through the existing API key flow without widening the
branch into broader relation mutation.

## Product decision

Skill-only taxonomy assignment ships before broader relation mutation.

This is the right next move because:

- it completes the taxonomy-mutation symmetry across the primary graph entity
  types
- skills already have a stable admin taxonomy workflow to mirror
- it reuses the existing `skill_taxonomy_terms` join model directly
- it clears the way for relation mutation without leaving one entity family
  behind
...
- an empty array is valid and clears the skill record’s taxonomy joins
- all referenced taxonomy terms must exist
- all referenced taxonomy terms must have `scope = skill`
...
- `entity: "ingest:skill-taxonomy"`
- `data.kind: "skill-taxonomy"`
- `data.skillId`
...
Additional validation failures also use stable responses:

- missing skill record -> `404`
- unknown taxonomy term IDs -> `404`
- taxonomy term scope mismatch -> `400`
...
5. The service verifies the skill record exists.
...
- `lib/ingestion/skill-taxonomy-assignment.ts`
...
- verify taxonomy term existence and `scope = skill`
...
- `skill_taxonomy_terms` for join storage
- `ingestion_events` for replay tracking
- `buildErrorEnvelope()` and `jsonOk()` for stable responses
...
- one request describes the full desired skill taxonomy state
...
- trusted publishers can replace skill taxonomy assignments with
  `POST /api/v1/ingest/skill-taxonomy`
...
- broader relation mutation across the graph
