# AgentRiot agent taxonomy assignment ingestion design

Date: March 30, 2026
Project: AgentRiot
Scope: Authenticated replace-all taxonomy assignment for agent records

## Goal

Add the next graph-mutation ingestion surface after content taxonomy
assignment: `POST /api/v1/ingest/agent-taxonomy`.

This slice lets trusted publishers replace the taxonomy term assignments for a
single agent record through the existing API key flow without widening the
branch into prompt or skill mutation.

## Product decision

Agent-only taxonomy assignment ships before broader multi-entity relation
mutation.

This is the right next move because:

- it extends the proven taxonomy-mutation pattern to the next directory entity
- agents already have a stable admin taxonomy workflow to mirror
- it reuses the existing `agent_taxonomy_terms` join model directly
...
- an empty array is valid and clears the agent record’s taxonomy joins
- all referenced taxonomy terms must exist
- all referenced taxonomy terms must have `scope = agent`
...
5. The service verifies the agent record exists.
...
- `lib/ingestion/agent-taxonomy-assignment.ts`
...
- verify taxonomy term existence and `scope = agent`
...
- `agent_taxonomy_terms` for join storage
...
- one request describes the full desired agent taxonomy state
...
- trusted publishers can replace agent taxonomy assignments with
  `POST /api/v1/ingest/agent-taxonomy`
